...
const fetchModels=async()=>{
  ...
  const handleShowProductView = (productid, qt) => {
    props.history.push({ pathname: `/ProductView/${productid}` }, qt);
  };
    ...
    return (
      <FormControl component="fieldset" >
        ...
        {getModelList.map((item)=>{
          return(
            <FormControlLabel
            control={<Checkbox checked={item.checked} style={{color:'#de011b'}} size="small" onChange={handleModelChange} name={item.modelid} />}
            label={<span style={{ fontSize: 12 }}>{item.modelname}</span>}
          />
          )
        })
        }
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} sm={6} style={{ padding: 5 }}>
              <img
                ...
                src={`${ImageURL}/images/${getBrand.ad}`}
